...
void executeUserJourney(const UserJourneyQuery &q, const HashIndex &hashIdx,
  ...
  if (userRecordCount > 0) {
    ...
    for (int i = 0; i < userRecordCount; i++) {
      if (userRecords[i]->timestamp >= q.timeStart &&
          userRecords[i]->timestamp <= q.timeEnd) {
  ...
  if (filteredCount == 0) {
    ...
    for (int i = 0; i < filteredCount; i++) {
      ...
      printf("[%s] %s → %s → %s (%s @ %s)\n", timestamp.c_str(),
             record->device_id.c_str(), record->app_id.c_str(),
             record->resource_id.c_str(), eventStr.c_str(), locStr.c_str());
    }
