...
void runAnomalyDetection(AnomalyList &results, const DataStore &store,
  ...
  std::cout << "[INFO] Sorting " << rawResults.count << " anomalies..."
            << std::endl;
...
void printAnomalyReport(const AnomalyList &results) {
  ...
  for (int i = 0; i < 14; i++) {
    if (typeCounts[i] > 0) {
      ...
      std::cout << std::left << std::setw(25) << getAnomalyTypeName(type)
                << std::setw(8) << typeCounts[i] << getRiskLevel(type) << "\n";
    }
  }
  ...
  std::cout << "\nTOP " << (userCountSize < 10 ? userCountSize : 10)
            << " SUSPICIOUS USERS\n";
  std::cout << "───────────────────────────────────────────────────────────────"
               "────────────────\n";
  ...
  for (int i = 0; i < topCount; i++) {
    std::cout << std::left << std::setw(8) << (i + 1) << std::setw(12)
              << userCounts[i].userId << userCounts[i].count << "\n";
  }
