...
LoadResult load(const std::string &filepath, DataStore &store) {
  ...
  while (std::getline(file, line)) {
    ...
    if (tokenCount != 7) {
      std::cerr << "[WARNING] Line " << lineNum << ": Expected 7 fields, got "
                << tokenCount << std::endl;
      ++result.skipped;
      delete[] tokens;
      continue;
    ...
    try {
      ...
      std::cerr << "[WARNING] Line " << lineNum
                << ": Invalid timestamp: " << timestamp_str << std::endl;
      ++result.skipped;
      delete[] tokens;
      continue;
    }
    ...
    // Tạo LogRecord
    LogRecord *record = new LogRecord(user_id, device_id, app_id, resource_id,
                                      parseEventType(event_str),
