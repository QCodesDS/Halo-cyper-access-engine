/**
 * @file        HashTable.h
 * @brief       Định nghĩa cấu trúc dữ liệu bảng băm (Hash Table) xử lý va chạm bằng Separate Chaining.
 *
 * Cung cấp giải pháp lưu trữ các con trỏ bản ghi log theo cơ chế băm chuỗi nội bộ, 
 * hỗ trợ cấp phát động mảng giá trị cho các khóa trùng lặp.
 */
...
/**
 * @struct HashNode
 * @brief  Nút trong danh sách liên kết đơn phục vụ kỹ thuật giải quyết va chạm băm (Separate Chaining).
 */
struct HashNode
{
...
struct HashTable
{
    HashNode **buckets; ///< Mảng các con trỏ bucket (mỗi bucket quản lý một danh sách liên kết)
    int tableSize;      ///< Tổng số lượng bucket của bảng băm
    int totalKeys;      ///< Tổng số lượng khóa duy nhất (Unique keys) hiện có trong bảng băm
};
 ...
 * @return Mảng các con trỏ trỏ đến các LogRecord hợp lệ, hoặc trả về nullptr nếu khóa không tồn tại
 */
LogRecord **lookupHash(const HashTable &ht, const std::string &key, int &outCount);

/**
 * @brief Giải phóng toàn bộ các nút liên kết và các mảng động lưu trữ giá trị trong bảng băm.
 * @param ht Tham chiếu tới bảng băm cần giải phóng bộ nhớ
 */
void clearHashTable(HashTable &ht);

/**
 * @brief Hàm băm cuốn đa thức (Polynomial Rolling Hash Function) chuyển đổi chuỗi thành chỉ mục số.
