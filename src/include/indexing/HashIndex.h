/**
 * @file        HashIndex.h
 * @brief       Định nghĩa cấu trúc chỉ mục băm đa chiều (Hash Index) cho hệ thống log.
 *
 * Cung cấp giải pháp lập chỉ mục đồng thời trên 3 trường thông tin cốt lõi: user_id, 
 * device_id và resource_id nhằm tối ưu hóa tốc độ truy vấn đa chiều cho các bộ dò tìm.
 */
...
/**
 * @brief Truy vấn danh sách các bản ghi log dựa trên mã định danh người dùng (User ID).
 ...
 * @return Mảng các con trỏ trỏ đến LogRecord kết quả, trả về nullptr nếu không tìm thấy
 */
LogRecord **getByUser(const HashIndex &idx, const std::string &userId, int &outCount);

/**
 * @brief Truy vấn danh sách các bản ghi log dựa trên mã định danh thiết bị (Device ID).
 * @param idx Tham chiếu tới cấu trúc chỉ mục băm đa chiều
...
/**
 * @brief Truy vấn danh sách các bản ghi log dựa trên mã định danh tài nguyên (Resource ID).
 * @param idx Tham chiếu tới cấu trúc chỉ mục băm đa chiều
...
/**
 * @brief Xóa bỏ và giải phóng toàn bộ tài nguyên, bộ nhớ của cấu trúc chỉ mục băm đa chiều.
