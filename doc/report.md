...
Dự án đã hoàn thành **100%** các yêu cầu chức năng từ FR-01 đến FR-09 theo đặc tả trong tài liệu BRD.md, tuân thủ tuyệt đối quy định không dùng các cấu trúc dữ liệu STL (như `std::vector`, `std::map`, `std::set`, `std::unordered_map`).
